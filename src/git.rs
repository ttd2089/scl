...
impl ConventionalCommitFooter {
    fn new(footer: &git_conventional::Footer) -> ConventionalCommitFooter {
        ...
        let value = footer.value().to_owned();
        let separator = Self::map_separator(footer);
        let raw = format!("{}{}{}", &token, separator, &value).to_owned();
    ...
    fn map_separator(footer: &git_conventional::Footer) -> String {
        match footer.separator() {
            git_conventional::FooterSeparator::Value => ": ".to_owned(), // FooterSeparator is defined as ":" instead of ": "
            x => x.to_string(),
        }
    }
}
